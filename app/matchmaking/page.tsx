...
def build_team_format_vote_embed(match_row, random_votes: int, captains_votes: int, voted_count: int, total_players: int):
    embed = discord.Embed(
        ...
        description=(
            ...
            "Voting ends in 20 seconds, or as soon as everyone in the queue has voted.\n"
...
class JoinQueueView(discord.ui.View):
    ...
    async def refresh_message(self, interaction: discord.Interaction):
        ...
        if match_row["status"] == "team_format_vote":
            ...
            vote_view.start_timer()  # starts the guaranteed 20-second countdown
            return
...
class TeamFormatVoteView(discord.ui.View):
    ...
    every queued player has voted, or after 20 seconds, whichever
    ...
    NOTE: discord.py's built-in View timeout is unreliable when the view
    is attached via message.edit() without bot.add_view(). We use an
    explicit asyncio.Task (_timeout_task) to guarantee the 20-second
    deadline fires. Call start_timer() right after sending/editing the
    message so the task has access to the guild.
    """

    VOTE_TIMEOUT = 20  # seconds

    def __init__(self, cog: "MatchmakingCog", match_number: int, total_players: int):
        super().__init__(timeout=None)  # disable discord.py's unreliable built-in timer
        self.cog = cog
        self.match_number = match_number
    ...
    def start_timer(self) -> None:
        """
        Schedule the forced resolution after VOTE_TIMEOUT seconds.
        Call this immediately after the vote embed is sent/edited so
        the asyncio event loop starts counting from the right moment.
        """
        self._timeout_task = asyncio.get_event_loop().create_task(self._run_timer())

    async def _run_timer(self) -> None:
        """Wait VOTE_TIMEOUT seconds, then force-resolve if not already done."""
    ...
    async def _register_vote(self, interaction: discord.Interaction, choice: str):
        ...
        async with lock:
            ...
            if len(self.votes) >= self.total_players:
                # Cancel the pending timer — we're resolving early.
